...
SAVE_DIR=/scratch4/danielk/tli104/mnmt/checkpoints/opus_en_x_8_ent_threshold
mkdir -p $SAVE_DIR

TRAIN_DATA_DIR=/scratch4/danielk/tli104/opus-100-corpus/v1.0/supervised/data-bin

python3 train.py $TRAIN_DATA_DIR --arch transformer_iwslt_de_en \
    ...
    --criterion label_smoothed_cross_entropy_token --label-smoothing 0.1 \
    --optimizer adam --adam-eps 1e-06 --adam-betas "(0.9, 0.98)" \
    --lr-scheduler inverse_sqrt --lr 0.0005 --warmup-updates 4000 --max-update 64000 \
    --no-epoch-checkpoints \
    --dropout 0.1 --attention-dropout 0.1 --weight-decay 0.0 --max-tokens 16384 --update-freq 8 \
    --log-format simple --log-interval 100 --seed 12 --fp16 --ddp-backend no_c10d \
    --save-dir $SAVE_DIR --max-source-positions 256 --max-target-positions 256 \
    --skip-invalid-size-inputs-valid-test --wandb-project paracrawl-en-$1 \
    --prune-metric el2n \
    --token-prune-threshold $1 \
    --start-prune-iteration $2
