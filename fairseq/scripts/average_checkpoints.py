...
def average_checkpoints(inputs, alpha=0.5):
    ...
    assert num_models == 2, "only supports interpolating between two models"

    for index, fpath in enumerate(inputs):
        print(index, fpath)
        ...
        for k in params_keys:
            ...
            if k not in params_dict:
                params_dict[k] = p.clone()*alpha
                # NOTE: clone() is needed in case of p is a shared parameter
            else:
                params_dict[k] += p*(1-alpha)
...
def main():
    ...
    new_state = average_checkpoints(args.inputs, alpha=args.alpha)
