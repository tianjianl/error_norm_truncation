...
@register_task("translation_multi_simple_epoch_print_grad")
class TranslationMultiSimpleEpochPrintGradTask(LegacyFairseqTask):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--dev-lang', type=str, default='fr')
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets['valid_by_lang'] = {
            k.split('-')[-1]: d for k, d in self.data_manager.load_split_datasets(
    ...
    def valid_step(self, sample, model, criterion):
        model.eval()
        with torch.no_grad():
            loss, _, sample_size, logging_output = criterion(model, sample)
    ...
    def train_step(
        ...
        # get gradient of each development set
         
        """
        for lang in self.datasets['valid_by_lang']:
            dataset = self.datasets['valid_by_lang'][lang]
            ...
            #print(f"lang = {lang} | number of dev batches = {len(itr)}")

            for sample in itr:
                if sample is None or len(sample) == 0:
                    sample = self.dummy_batch
                    dev_ignore_grad = True
                else:
                    dev_ignore_grad = False
            ...
            grad = self._flatten_grad(model, optimizer, zero_grad=True)
            # dev_grads.append(grad)
        
        #for lang1, grad1 in dev_grads:
        #    for lang2, grad2 in dev_grads:
        #        cos_sim = cosine_similarity(grad1, grad2, dim=0)
        #        print(f"cosine similarity between {lang1} and {lang2} = {cos_sim}")

        """
        
        dev_dataset = self.datasets['valid_by_lang'][self.dev_lang]
        dev_itr = self.get_batch_iterator(
            dataset=dev_dataset,
            max_tokens=16384,
            max_positions=utils.resolve_max_positions(
                self.max_positions(),
        ...
        for dev_sample in dev_itr:
            if dev_sample is None or len(dev_sample) == 0:
                dev_sample = self.dummy_batch
                dev_ignore_grad = True
            else:
                dev_ignore_grad = False
                
            dev_sample = self._prepare_sample(dev_sample)
            dev_loss, _, _, _ = criterion(model, dev_sample, reduce=True)
            if dev_ignore_grad:
                dev_loss *= 0
            dev_loss = dev_loss.sum()
            dev_loss.backward(retain_graph=True)
        
        dev_grads = {}
        for n, p in model.named_parameters():
            if p.requires_grad:
                dev_grads[n] = p.grad.detach().clone()
        
        optimizer.zero_grad()

        with torch.autograd.profiler.record_function("forward"):
            with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                loss, batched_loss, sample_size, logging_output = criterion(model, sample, reduce=True)
                
        if ignore_grad:
            loss *= 0

        # this implements equation (7) in https://arxiv.org/abs/1911.10088
        model_copy = copy.deepcopy(model)
        for n, p in model_copy.named_parameters():
            if p.requires_grad:
                p.data.copy_(p.data + eps * dev_grads[n])
            
        _, new_loss, _, _ = criterion(model_copy, sample, reduce=True) 
        per_example_similarity = (new_loss - batched_loss) / eps
        
        # per_example_similarity should be batch size, length

        per_example_similarity = torch.squeeze(per_example_similarity, dim=-1)
        per_example_similarity = torch.mean(per_example_similarity, dim=-1).unsqueeze(dim=-1)
        # this is of shape (batch size, 1)
        
        # if idx == 0:
        #    similarity_by_language = per_example_similarity
        # else:
        #    similarity_by_language = torch.cat((similarity_by_language, per_example_similarity), dim=1)
        
        del model_copy
        gc.collect()

        # print samples and their respective gradient similarity to development set
        with open(f'/scratch/tli104/sim_gradient_result_{self.dev_lang}.txt', 'a+') as f:
            #print(f"update num = {update_num}", file=f)
            for src, tgt, sim_score in zip(sample["net_input"]["src_tokens"], sample["target"], per_example_similarity):
                print(' '.join([str(tok_id) for tok_id in src.tolist()]), file=f)
                print(' '.join([str(tok_id) for tok_id in tgt.tolist()]), file=f)
                print(sim_score.item(), file=f)
                
        # for debugging: printing everything to the console
        # for src, tgt, sim_score in zip(sample["net_input"]["src_tokens"], sample["target"], per_example_similarity):
        #    print(' '.join([str(tok_id) for tok_id in src.tolist()]))
        #    print(' '.join([str(tok_id) for tok_id in tgt.tolist()]))
