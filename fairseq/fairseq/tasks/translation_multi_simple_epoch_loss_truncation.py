...
@register_task("translation_multi_simple_epoch_loss_truncation")
class TranslationMultiSimpleEpochLossTruncationTask(LegacyFairseqTask):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--loss-truncation', type=float, default=0, help='the porpotion of data to do loss truncating')
    ...
    def __init__(self, args, langs, dicts, training):
        ...
        self.loss_truncation = args.loss_truncation
    ...
    def train_step(
        ...
        with torch.autograd.profiler.record_function("forward"):
            with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                #standard training
                loss, sample_size, logging_output = criterion(model, sample, dropc=self.loss_truncation)
        if ignore_grad:
            loss *= 0
