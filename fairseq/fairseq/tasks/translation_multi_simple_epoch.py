...
class TranslationMultiSimpleEpochTask(LegacyFairseqTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--token-prune-threshold', default=None, type=float, 
                            help='the el2n score threshold above which tokens with a el2n score will be pruned.')
        parser.add_argument('--start-prune-iteration', default=0, type=int, 
                            help='the iteration that you want to start to do token pruning')
        parser.add_argument('--prune-metric', default='el2n', choices=['el2n', 'prob'],
                            help='the metric for pruning data: el2n or prob')
        parser.add_argument('--prune-fraction', default=0, type=float, help="the fraction of data to prune according to metric, cannot be jointly used when --token-prune-threshold is set")
        parser.add_argument('--length-prune', type=float, default=0, help='the porpotion of length after which is only being trained')
        parser.add_argument('--mask-input', default=None, type=str, 
                            help='masking the input tokens that are hard to predict, if set to "both", then target is also masked')
        parser.add_argument('--gamma', default=0, type=float)
        parser.add_argument('--prune-languages', default='all', type=str, help='list of languages to prune, seperated by comma')
    ...
    def __init__(self, args, langs, dicts, training):
        ...
        self.token_prune_threshold = args.token_prune_threshold
        self.prune_fraction = args.prune_fraction

        if self.token_prune_threshold != None and self.prune_fraction != 0:
            raise Exception("only one of --token-prune-threshold and --prune-fraction should be set")
        self.start_prune_iteration = args.start_prune_iteration 
        self.prune_metric = args.prune_metric
        self.length_prune = args.length_prune
        self.mask_input = args.mask_input
        self.gamma = args.gamma 
        
        self.prune_languages = [self.get_langtok(lang) for lang in args.prune_languages.split(',')] if args.prune_languages != 'all' else 'all' 
        #print(f"prune_languages = {self.prune_languages}")
    ...
    def get_langtok(self, lang):
        
        tgt_lang_tok = self.data_manager.get_decoder_langtok(lang, 'placeholder')
        return tgt_lang_tok
    ...
    def train_step(
        ...
        ignore_low_probs = None if update_num <= self.start_prune_iteration else self.token_prune_threshold
        prune_fraction = 0 if update_num <= self.start_prune_iteration else self.prune_fraction 
        
        if self.gamma != 0:
            # using Tailr, setting token prune to none
            ignore_low_probs = None

        # for calculating el2n after each epoch 
        with torch.autograd.profiler.record_function("forward"):
            with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                if ignore_low_probs == None and prune_fraction == 0:
                    if self.gamma == 0: 
                        #standard training
                        loss, sample_size, logging_output = criterion(model, sample)
                    else: #tailr in https://arxiv.org/abs/2302.13344 
                        loss, sample_size, logging_output = criterion(model, sample, gamma=self.gamma)

                else:
                    loss, sample_size, logging_output = criterion(model, sample, 
                                            ignore_low_probs=ignore_low_probs, 
                                            metric=self.prune_metric, 
                                            length_prune=self.length_prune,
                                            mask_input=self.mask_input, 
                                            prune_languages=self.prune_languages,
